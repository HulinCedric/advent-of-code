...
public class HandShould
{
    ...
    public void HighCard_Should_Be_Weaker_Than_OnePair(string firstHand, string secondHand)
        => HandParserWithJAsJack.Parse(firstHand).Should().BeLessThan(HandParserWithJAsJack.Parse(secondHand));
    ...
    public void Same_HandType_should_be_compared_by_Ordered_Card_Strength(
        string strongerHand,
        string weakerHand)
        => HandParserWithJAsJack.Parse(strongerHand).Should().BeLessThan(HandParserWithJAsJack.Parse(weakerHand));
}