...
public class HandShould
{
    ...
    public void HighCard_Should_Be_Weaker_Than_OnePair(string firstHand, string secondHand)
        => ParserWithJAsJack.Parse(firstHand).Should().BeLessThan(ParserWithJAsJack.Parse(secondHand));
    ...
    public void Same_HandType_should_be_compared_by_Ordered_Card_Strength(
        string strongerHand,
        string weakerHand)
        => ParserWithJAsJack.Parse(strongerHand).Should().BeLessThan(ParserWithJAsJack.Parse(weakerHand));
}